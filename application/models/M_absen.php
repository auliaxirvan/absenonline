...
class M_absen extends CI_Model
{
    ...
    // Tambahan Febri (Tabel Upcara)
    // public function jadwal_upacara($tgl, $tpp_dept_id, $user_id = NULL)
    // {
    //     $query = $this->db->select('sch_upacara.id, sch_upacara.tanggal, sch_upacara.jam_mulai, sch_upacara.ket')
    //         // ->join('apel_pegawai', 'sch_upacara.id = apel_pegawai.sch_upacara_id', 'left')
    //         ->where('tanggal', $tgl);
    //     // ->where('sch_upacara.deleted', 1);
    //     if ($user_id != NULL) {
    //         $query = $this->db->where('apel_pegawai.user_id', $user_id);
    //     }
    //     // $query = $this->db->where("'$tpp_dept_id'", 'ANY(sch_upacara.dept_id)', false)
    //     //     ->get('sch_upacara')->row();
    //     $query = $this->db->get('sch_upacara')->row();
